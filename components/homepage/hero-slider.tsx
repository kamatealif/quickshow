// components/homepage/hero-slider.tsx
"use client";
...
export default function HeroSlider({ movies }: { movies: Movie[] }) {
  ...
  // Helper to handle mixed image sources (Wikipedia vs TMDB)
  const getImageUrl = (path: string | undefined) => {
    if (!path) return "/placeholder-backdrop.jpg";
    if (path.startsWith("http")) return path; // Returns Wikipedia/External links directly
    return `https://image.tmdb.org/t/p/original${path}`; // Handles TMDB paths
  };
    ...
    <section className="relative h-dvh w-full bg-black">
          ...
          <SwiperSlide key={movie.id}>
            <div className="relative h-full w-full overflow-hidden">
              <Image
                ...
                priority={index === 0}
                unoptimized={movie.backdrop_path?.includes("wikimedia")} // Recommended for Wikipedia links
              />
      ...
      <style jsx global>{`
        ...
        .custom-pagination .swiper-pagination-bullet-active {
          width: 36px;
          opacity: 1;
          background: #e11d48 !important; /* Use your primary color hex */
          border-radius: 20px;
        }
      `}</style>
    </section>
